...
const Card = ({item}) => {
  return (
    <>
    <div key={item.id} className={`w-full h-full relative ${item.id == 1 && "md:col-span-2 md:row-span-2"}  ${item.id == 2 && "md:col-span-2 md:row-span-1"}`}>
        <Image className="h-full w-full" src={item.Image} alt={item.name} />
        <div className={` ${item.id == 4? "w-4/5": "w-1/2"} md:w-auto absolute bottom-7.5 md:bottom-10 left-7.5 md:left-10`}>
          {item.id !== 4 && <h4 className='font-jost text-primary-black md:text-sm font-normal leading-6 '>HOT LIST</h4>}
          <h2 className='text-[22px] md:text-[24px] font-jost font-medium'><span className='font-bold'>{item.name}</span> {item.id === 4 ? "CARDS" : "COLLECTION"}</h2>
